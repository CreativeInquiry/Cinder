...
GLuint CompileProgram(const std::string &vsSource, const std::string &fsSource)
{
	...
	if (program == 0)
	{
		CI_LOG_E("Program creation failed");
	}
	...
	mDrawCount(0)
{
	...
	float halfWidth = isHolographic ? 0.2f : 0.5f;
...
{
	mCubeRenderer->UpdateWindowSize(getWindow()->getWidth(), getWindow()->getHeight());
}
